...
Modes (matched by the standard adaptive-trigger vocabulary):
  ...
  bow         resistance curve     (0x22, ramp curve)
  machine gun rapid vibration      (0x26, fast pulse oscillation)

Examples:
  triggers.py /dev/hidraw4 strong off
  triggers.py /dev/hidraw4 medium medium
  triggers.py auto bow bow 0.1 0.85
  triggers.py auto "machine gun" "machine gun" 0.0 0.90
...
MODES = {
    ...
    "pulse": ("pulse", 0),
    "bow": ("bow", 0xC0),
    "machine gun": ("machine_gun", 0xE0),
    "machinegun": ("machine_gun", 0xE0),
}
...
def encode_trigger(mode: str, force: int, start: int = 0) -> bytes:
    """Encode adaptive trigger effect parameters into an 8-byte block."""
    if mode == "continuous":
        return struct.pack("<BBHHH", EFFECT_CONTINUOUS, start, force, 0, 0)
    if mode == "rigid":
        return struct.pack("<BBHHH", EFFECT_RIGID, start, force if force else 0, 0, 0)
    if mode == "pulse":
        return struct.pack("<BBHHH", EFFECT_PULSE, start, 0x0064, force if force else 0x0032, 0)
    if mode == "bow":
        # Bow effect: resistance curve ramp starting at start position with ramped force
        return struct.pack("<BBHHH", EFFECT_BOW, start, force if force else 0x00C0, 0x0080, 0)
    if mode in ("machine_gun", "machinegun"):
        # Machine Gun: rapid vibration pulse effect (high frequency rapid pulses)
        return struct.pack("<BBHHH", EFFECT_PULSE, start, 0x00C8, force if force else 0x00E0, 0)
    return struct.pack("<BBHHH", EFFECT_OFF, 0, 0, 0, 0)


def build_report(left: str, right: str, start_pos: int = 0, force_override: int = None) -> bytes:
    """DualSense USB output report 0x02 with adaptive trigger bytes configured."""
    lmode, lforce = MODES.get(left, ("off", 0))
    rmode, rforce = MODES.get(right, ("off", 0))
    if force_override is not None:
        if lmode != "off":
            lforce = force_override
        if rmode != "off":
            rforce = force_override
    ...
    report[5] = 0x20
    enc_l = encode_trigger(lmode, lforce, start_pos)
    enc_r = encode_trigger(rmode, rforce, start_pos)
    report[10:10 + len(enc_l)] = enc_l   # left trigger block
    report[10 + len(enc_l):10 + len(enc_l) + len(enc_r)] = enc_r   # right trigger block
    return bytes(report)


def main() -> None:
    if len(sys.argv) < 4:
        fail("usage: triggers.py <hidraw-node|auto> <left-mode> <right-mode> [start_pos] [force]")
    node = sys.argv[1]
    ...
    if len(sys.argv) > 4:
        try:
            s_val = float(sys.argv[4])
            if s_val > 1.0:
                s_val = s_val / 255.0 if s_val > 100.0 else s_val / 100.0
            start_pos = int(round(max(0.0, min(1.0, s_val)) * 255))
        except ValueError:
            start_pos = 0

    if len(sys.argv) > 5:
        try:
            f_val = float(sys.argv[5])
            if f_val > 1.0:
                f_val = f_val / 255.0 if f_val > 100.0 else f_val / 100.0
            force_override = int(round(max(0.0, min(1.0, f_val)) * 255))
        except ValueError:
            force_override = None
